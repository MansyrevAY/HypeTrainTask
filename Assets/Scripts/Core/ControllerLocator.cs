...
public class ControllerLocator : MonoBehaviour, IServiceLocator
{
    ...
    private const string pluginName = "com.mansyrev.unity.TimerPlugin";

    static AndroidJavaClass _pluginClass = new AndroidJavaClass(pluginName);
    static AndroidJavaObject _pluginInstance = _pluginClass.CallStatic<AndroidJavaObject>("getInstance");

    double GetTime()
    {
        if (Application.platform == RuntimePlatform.Android)
            return _pluginInstance.Call<double>("GetTime");
        else
            return -17.34;
    }
